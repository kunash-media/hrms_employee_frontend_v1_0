...
// Live location ping tracking — NEW
let livePingInterval     = null;
let currentAttendanceId  = null;
const LIVE_PING_INTERVAL_MS = 5 * 60 * 1000; // 5 min — change to 10*60*1000 for 10 min
...
async function apiCheckIn() {
  ...
  const payload = {
    ...
    checkInTime:     now,
    checkOutTime:    null,
    status:          selectedCheckInStatus || 'Present',
    notes:           'Regular working day'
  };
  const res = await fetch(`${BASE_URL}/api/attendance/check-in`, {
    method:  'POST',
  ...
  return { checkInTime: now, attendanceId: data.attendanceId };
} 
...
/* ── Geofencing — capture browser location ──────────────── */
function getCurrentCoords(timeoutMs = 8000) {
...
async function apiCreateGeoLog(attendanceId, coords, logType = 'CHECK_IN') {
  ...
  const res = await fetch(`${BASE_URL}/api/attendance-geo/log`, {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(payload)
  });
  if (!res.ok) throw new Error(`Geo log failed (${res.status})`);
  return res.json();
}

/* ── Live location ping loop — runs while checked in ────── */
function startLivePingLoop(attendanceId) {
  stopLivePingLoop(); // clear any stale interval first

  livePingInterval = setInterval(() => {
    console.log(`[Attendance] Live location ping tick at ${new Date().toLocaleTimeString('en-IN')}`);
    getCurrentCoords()
      .then(coords => apiCreateGeoLog(attendanceId, coords, 'LIVE'))
      .then(() => console.log('[Attendance] Live location ping saved'))
      .catch(err => console.warn('[Attendance] Live ping skipped:', err.message));
  }, LIVE_PING_INTERVAL_MS);

  console.log(`[Attendance] Live ping loop started — every ${LIVE_PING_INTERVAL_MS / 60000} min`);
}

function stopLivePingLoop() {
  if (livePingInterval) {
    clearInterval(livePingInterval);
    livePingInterval = null;
    console.log('[Attendance] Live ping loop stopped');
  }
}
...
document.getElementById('checkInBtn').addEventListener('click', async () => {
  ...
  document.getElementById('checkInBtn').disabled = true;
  try {
    const { checkInTime, attendanceId } = await apiCheckIn();
...
async function init() {
 ...
 const sess = loadSession();
  if (sess) {
    ...
    // Resume live location ping loop after page reload mid-session — NEW
    await fetchAllRecords();
    const todayRec = getTodayRecord();
    if (todayRec && todayRec.attendanceId) {
      currentAttendanceId = todayRec.attendanceId;
      startLivePingLoop(currentAttendanceId);
    }
  }
